...
from config  import C_REPO_NAME, C_ORGA_NAME, C_OWNER_NAME, C_REPO_STARS, GITHUB_REPO_SCORE_FILEPATH, C_REPO_LAST_MODIFIED
...
MIN_STARS = 5
BIG_COMPANY_LIST = [
    "Microsoft","Meta Research", "Google", "BCG Gamma", "BCG-Gamma","Amazon", "IBM", "salesforce",
    "facebookresearch", "Netflix", "AstraZeneca", "DeepMind", "PAIR", "LinkedIn", "tensorflow"
                 ]
...
def add_is_big_company(repositories):
    ...
    mask = (
        (repositories[C_REPO_NAME].str.contains(regex_big_company, na=False, flags=re.IGNORECASE ))
        | (repositories[C_OWNER_NAME].str.contains(regex_big_company, na=False, flags=re.IGNORECASE ))
        | (repositories[C_ORGA_NAME].str.contains(regex_big_company, na=False, flags=re.IGNORECASE))
...
def add_years_since_creation(repositories):
    return repositories.assign(
        **{
        C_REPO_YEARS_SINCE_CREATION: (pd.Timestamp.today() - repositories.repo_created_at).dt.days/365
    }
    )
...
def add_years_since_last_modification(repositories):
    return repositories.assign(
        **{
            C_REPO_YEARS_SINCE_MODIFICATION: (pd.Timestamp.today() - repositories[C_REPO_LAST_MODIFIED]).dt.days/365
        }
    )
...
def add_features(repositories):
    return repositories.pipe(
                ...
                add_years_since_last_modification
            )
...
if __name__ == '__main__':
    ...
    candidates = repositories[repositories[C_SCORE] > 0].sort_values(by=C_SCORE, ascending=False).reset_index(drop=True)
    print(candidates[candidates[C_IS_BIG_COMPANY] == 0].groupby(["orga_name"])[C_SCORE].sum().sort_values(ascending=False).head(50))
    print(candidates[candidates[C_IS_BIG_COMPANY] == 0].groupby(["owner_name", "repo_size"])[C_SCORE].sum().sort_values(ascending=False).head(50))
